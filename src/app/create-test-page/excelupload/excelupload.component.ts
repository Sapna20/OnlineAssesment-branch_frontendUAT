...
export class ExceluploadComponent implements OnInit {
  ...
  predinedlevels = AppConstants.questionDifficultyLevel;
  predinedAnswer = ['a', 'b', 'c', 'd'];
  predifinedSubjects = AppConstants.testTopic; //['Maths', 'Chemistry', 'Physics', 'Management', 'Coding', 'Machne Learning'];
  ...
  onSubmit(){
    ...
    for(let i = 0;i<this.questionList.length;i++){
      ...
      question.id = String(this.questionList[i]['id']) + String( new Date());
      question.question = btoa(this.questionList[i]['question']);
      question.option1 = btoa(this.questionList[i]['option1']);
      question.option2 = btoa(this.questionList[i]['option2']);
      question.option3 = btoa(this.questionList[i]['option3']);
      question.option4 = btoa(this.questionList[i]['option4']);
    ...
    this.httpService.addQuestions(createQuestion).subscribe(data=>{
      //console.log(data);
    },
